    ...
    Users.findOne({
        ...
        if (!err && user) { // Found
            if (user.facebook === '') { // Account not yet attached
                ...
                res.send(`<Response><Message>Welcome back, we still need permission to access your Facebook account. http://40217540.ngrok.com/auth/facebook?phone=${req.body.From}</Message></Response>`);
            ...
            res.send(`<Response><Message>To get started, we need access to your Facebook account. http://40217540.ngrok.com/auth/facebook?phone=${req.body.From.replace("+","")}</Message></Response>`);
        }
    });
};
